...
int main ( void )
{
    ...
    for ( size_t i = 0; i < num_accounts; i++ )
    {
        ...
        account_list[i].email = malloc ( sizeof ( *account_list->email ) * strlen ( input_buffer ) );
        strcpy ( account_list[i].email, input_buffer );
        printf ( "enter acct[%d] name: ", i );
        scanf ( "%s", input_buffer );
        account_list[i].name = malloc ( sizeof ( *account_list->name ) * strlen ( input_buffer ) );
